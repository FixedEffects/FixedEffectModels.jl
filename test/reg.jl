...
# simple
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1)))  [139.73446,-0.22974] 1e-4
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @weight(w)))  [137.72495428982756,-0.23738] 1e-4

x[:SalesInt] = round(Int64, x[:Sales])
@test_approx_eq_eps coef(reg(x, @formula(SalesInt ~ Price))) [139.72674,-0.2296683205] 1e-4

# absorb
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1)))  [-0.20984] 1e-4
@test reg(x, @formula(y ~ x1), @fe(pid1)).iterations == 1

@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid2)))   [-1.08471] 1e-4
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid1&id2)))    [-0.53470] 1e-4
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1*id2)))    [-0.53470] 1e-4
#@test isempty(coef(reg(x, @formula(y ~ 0), @fe(pid1*x1))))
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(ppid1&pid2)))    [-1.44255] 1e-4

@test_approx_eq_eps  coef(reg(x, @formula(y ~ x1), @fe(pid1&id2)))  [13.993028174622104,-0.5804357763515606] 1e-4
@test_approx_eq_eps  coef(reg(x, @formula(y ~ x1), @fe(id2&pid1)))  [13.993028174622104,-0.5804357763515606] 1e-4
@test_approx_eq_eps  coef(reg(x, @formula(y ~ 1), @fe(id2&pid1)))  [174.4084407796102] 1e-4
@test_approx_eq_eps  coef(reg(x, @formula(y ~ x1), @fe(pid1&id2 + pid2&id1)))  [51.2358,-0.5797] 1e-4
@test_approx_eq_eps  coef(reg(x, @formula(y ~ x1 + x2), @fe(pid1&id2 + pid2&id1)))  [-46.4464,-0.2546, -0.005563] 1e-4
@test_approx_eq_eps  coef(reg(x, @formula(y ~ 0 + x1 + x2), @fe(pid1&id2 + pid2&id1)))   [-0.21226562244177932,-0.004775616634862829] 1e-4


# recheck these two below
@test_approx_eq_eps coef(reg(x, @formula(y ~ z1), @fe(x1&x2&pid1)))    [122.98713,0.30933] 1e-4
@test_approx_eq_eps coef(reg(x, @formula(y ~ z1), @fe((x1&x2)*pid1)))    [0.421406] 1e-4


# absorb + weights
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1), @weight(w))) [-0.21741] 1e-4
@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid2),  @weight(w)))  [-0.88794] 1e-3
...
# cluster
@test_approx_eq_eps stderr(reg(x, @formula(y ~ x1), @vcovcluster(pid1)))[2]  0.03792 1e-4
@test_approx_eq_eps stderr(reg(x, @formula(y ~ x1), @fe(pid1), @vcovcluster(pid2)))  [0.02205] 1e-4
# stata reghxe
@test_approx_eq_eps stderr(reg(x, @formula(y ~ x1), @fe(pid1), @vcovcluster(pid1)))  [0.03573] 1e-4
# no reference
@test_approx_eq_eps stderr(reg(x, @formula(y ~ x1), @vcovcluster(pid1 + pid2)))[1]  6.17025 1e-4
# no reference
@test_approx_eq_eps stderr(reg(x, @formula(y ~ x1), @fe(pid1), @vcovcluster(pid1 + pid2)))[1]   0.04037 1e-4
...
@test_approx_eq_eps coef(result)  coef(reg(x[x[:State] .<= 30, :], @formula(y ~ x1 + pid1))) 1e-4
@test_approx_eq_eps vcov(result)  vcov(reg(x[x[:State] .<= 30, :], @formula(y ~ x1 + pid1))) 1e-4
...
for method in [:cholesky, :qr, :lsmr]
	# absorb
	@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1), method = method))  [-0.11981270017206136] 1e-4
	@test_approx_eq_eps  coef(reg(x, @formula(y ~ x1), @fe(pid1&id2), method = method))   [-315.0000747500431,-0.07633636891202833] 1e-4
	#
	@test_approx_eq_eps  coef(reg(x, @formula(y ~ x1), @fe(id2&pid1), method = method))   [-315.0000747500431,-0.07633636891202833] 1e-4

	@test_approx_eq_eps  coef(reg(x, @formula(y ~ 1), @fe(id2&pid1), method = method))   [-356.40430526316396] 1e-4
	@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1), method = method, @weight(w)))  [-0.11514363590574725] 1e-4

	# absorb + weights
	@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid2), method = method))    [-0.04683333721137311] 1e-4
	@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid2), method = method,  @weight(w)))   [-0.043475472188120416] 1e-3

	## the last two ones test an ill conditioned model matrix
	@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid1&id2), method = method))     [-0.122354] 1e-4
	@test reg(x, @formula(y ~ x1), @fe(pid1 + pid1&id2), method = method).iterations <= 30

	@test_approx_eq_eps coef(reg(x, @formula(y ~ x1), @fe(pid1 + pid1&id2), method = method, @weight(w)))  [-0.11752306001586807] 1e-4
