...
@test convert(Matrix{Float64}, partial_out(df, @model(Sales + Price ~ NDI, fe = pState))[1])[1:5, :] ≈ [ -21.3715  -0.642783; -19.6571  -0.540335; -16.3427  -1.63789; -18.2631  -0.856975; -18.9784  -1.70283] atol = 1e-3
@test convert(Matrix{Float64}, partial_out(df, @model(Sales + Price ~ 1, fe = pState))[1])[1:5, :] ≈  [-13.5767   -40.5467; -12.0767   -39.3467; -8.97667  -39.3467; -11.0767   -37.6467; -11.9767   -37.5467] atol = 1e-3
@test convert(Matrix{Float64}, partial_out(df, @model(Sales + Price ~ 1))[1])[1:5, :] ≈ [ -30.0509  -40.0999; -28.5509  -38.8999; -25.4509  -38.8999; -27.5509  -37.1999; -28.4509  -37.0999] atol = 1e-3
@test mean(convert(Matrix{Float64}, partial_out(df, @model(Sales + Price ~ NDI, add_mean = true))[1]), dims = 1[1]) ≈ [123.951  68.6999] atol = 1e-3
