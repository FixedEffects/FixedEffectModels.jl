...
m = @model y ~ x1 + pid2
x = reg(df, m)
m = @model y ~ pid2 + fe(id1)
x = reg(df, m)
m = @model y ~ x1 + pid2 + fe(id1)
x = reg(df, m)
@test coef(x)[1] ≈ -1.08471 atol = 1e-4
m = @model y ~ x1 + pid2 + fe(id1)
x = reg(df, m,  weights = :w)
@test coef(x)[1] ≈ -0.88794 atol = 1e-4
m = @model y ~ x2 + (x1 ~ z1) + pid2 + fe(id1)
x = reg(df, m)
...
# iv + fe + cluster + weights - matches ivreghdfe
m = @model y ~ (x1 ~ z1) + fe(id1) vcov = cluster(id1)
x = reg(df, m,  weights = :w)
...
m=@model y ~ x1 + pid1 vcov=cluster(id1&id2)
x = reg(df, m)
...
m = @model y ~ z1 + CPI vcov = cluster(id1) subset = (State .>= 30)
x = reg(df, m)
...
m = @model y ~ (x1 ~ z1) vcov = cluster(id1)
x = reg(df, m)
...
m = @model y ~ (x1 ~ z1) + fe(id1) vcov = robust()
x = reg(df, m)
...
m = @model y ~ CPI + (x1 ~ z1) vcov = cluster(id1)
x = reg(df, m)
...
m = @model y ~ (x1 ~ z1 + CPI) vcov = cluster(id1 + id2)
x = reg(df, m)
...
m = @model y ~ x1 + fe(pn)  vcov = cluster(id1)
x = reg(df, m)
@test x.nobs == 60
...
df.id3 = df.id2 .>= 1980
m = @model y ~ x1 + fe(id1_missing) & fe(id3)
x = reg(df, m)
