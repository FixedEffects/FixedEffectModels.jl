...
Moreover `reg` returns a very light object.  It is simply composed of the coefficients, the covariance matrix, and a set of scalars like number of observations, degrees of freedoms, r2, etc. Usual methods `coef`, `vcov`, `nobs`, `predict`, `residuals` are still defined as long as you add the original dataframe as a second argument.
