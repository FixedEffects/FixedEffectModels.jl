...
## Syntax
To estimate a linear model, one needs to specify a formula with `@formula`, a set of fixed effects with `@fe`, a way to compute standard errors with `@vcov`, and a weight variable with `@weight`.
